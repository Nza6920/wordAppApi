...
public class AddUnskillWords extends RestfulApi {
    @Override
    public Object execute(JSONObject jreq) throws Exception {
        // 请求参数不正确, 返回403
        if (!ServiceUtils.jsonVerify(jreq, new String[]{"unskills"})) {
        ...
        JSONArray jsonArray;
        List<Word> unskills = new ArrayList<>();
        ...
        try {
            ...
            // 判断复习表是否存在(健壮性)
            if (unskilledId == null) {
                resultData.put("message", "该用户的生词表不存在");
                this.httpResp.setStatus(403);
                return resultData;
            }

            jsonArray = jreq.getJSONArray("unskills");
